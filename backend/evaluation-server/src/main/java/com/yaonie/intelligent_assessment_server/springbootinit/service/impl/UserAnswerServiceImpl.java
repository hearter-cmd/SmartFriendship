package com.yaonie.intelligent_assessment_server.springbootinit.service.impl;
...
import com.yaonie.intelligent_assessment_server.constant.CommonConstant;
import com.yaonie.intelligent_assessment_server.exception.ThrowUtils;
import com.yaonie.intelligent_assessment_server.model.dto.userAnswer.UserAnswerQueryRequest;
...
import com.yaonie.intelligent_assessment_server.model.enums.AppScoringStrategyEnum;
import com.yaonie.intelligent_assessment_server.model.enums.AppTypeEnum;
import com.yaonie.intelligent_assessment_server.model.vo.UserAnswerVO;
import com.yaonie.intelligent_assessment_server.model.vo.UserVO;
import com.yaonie.intelligent_assessment_server.springbootinit.mapper.UserAnswerMapper;
import com.yaonie.intelligent_assessment_server.springbootinit.service.AppService;
...
public class UserAnswerServiceImpl extends ServiceImpl<UserAnswerMapper, UserAnswer> implements UserAnswerService {
    ...
    public void validUserAnswer(UserAnswer userAnswer, boolean add) {
        ...
        if (appType != null) {
            ThrowUtils.throwIf(AppTypeEnum.getEnumByValue(appType) == null, ErrorCode.PARAMS_ERROR, "应用类型不存在");
        }
        if (scoringStrategy != null) {
            ThrowUtils.throwIf(scoringStrategy < 0 || scoringStrategy > 1, ErrorCode.PARAMS_ERROR, "评分策略错误");
        }
        if (StringUtils.isNotBlank(choices)) {
            ThrowUtils.throwIf(choices.length() > 1000, ErrorCode.PARAMS_ERROR, "选项过长");
        }
        if (resultId != null) {
            ThrowUtils.throwIf(scoringResultService.getById(resultId) == null, ErrorCode.PARAMS_ERROR, "结果ID不存在");
        }
    ...
    public QueryWrapper<UserAnswer> getQueryWrapper(UserAnswerQueryRequest userAnswerQueryRequest) {
        ...
        queryWrapper.eq(AppTypeEnum.getEnumByValue(appType) != null, "appType", appType);
        queryWrapper.eq(AppScoringStrategyEnum.getEnumByValue(scoringStrategy) != null, "scoringStrategy", scoringStrategy);
        queryWrapper.eq(ObjectUtils.isNotEmpty(choices), "choices", JSONUtil.toJsonStr(choices));
